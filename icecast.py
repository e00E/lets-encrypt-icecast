...
class IcecastConfiguration:
	...
	def __init__(self, filepath):
		...
		try:
			...
			raise PluginError("Icecast configuration file %s is malformed." % filepath)
	...
	def deploy_cert(self, domain, cert, key, cert_chain=None):
		...
		if not self.icecast_configuration.exists_ssl_socket():
			if self.create_ssl_socket:
				logger.info("Creating a new ssl socket with port %s" % self.default_ssl_port)
				self.icecast_configuration.make_ssl_socket(self.default_ssl_port)
				self.save_notes += "Created a new ssl socket\n"
			else:
				logger.info("Changing existing socket to use ssl")
				self.icecast_configuration.make_ssl_socket(self.default_ssl_port, self.icecast_configuration.find_first_socket_node())
				self.save_notes += "Converted existing socket to ssl socket\n"
		else:
			logger.info("An existing ssl socket was already found so no sockets were created or changed")
